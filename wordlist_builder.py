...
#           [ { "target": "word", "q": "qual" }, { "target": "word2", "tr": "tr" } ]
    ...
    def is_generated(cls, word, wordlist):
        ...
        for sense in word.senses:
            ...
            if word.pos == "part":
                # TODO: smart_inflection included temporarily to support 2022-12-01 database,
                # later databases should not have "smart_inflection" as a formtype of "part"
                if formtype in ["pp_ms", "smart_inflection"]:
                    lemma = sense.lemma
                else:
                    formtype = {"f": "pp_fs", "mpl": "pp_mp", "fpl": "pp_fp"}.get(formtype, formtype)
                    lemma = cls.get_part_lemma(sense.lemma, wordlist)
